...
| **Dashboard (Streamlit)** | `garmin\scripts\dashboard.py` — เปิดด้วย `run_dashboard.bat` (root) | 5 แท็บ: 👥 รวมทีม (สถานะ🟢🟡🔴+ACWR+ธงเฝ้าระวัง+**แถบเตือน sync ล่าสุดต่อคน**+**เตือนรองเท้าใกล้หมดสภาพทั้งทีม**), 💤 Health, 👟 Training (ACWR แถบสี + โดนัท 80/20), **📈 ความก้าวหน้า** (เทรนด์ VO2max + Garmin คาดการณ์ 5K/10K/HM/FM + Garmin LT + Endurance/Hill + PR + ตารางรองเท้า), 📉 Splits. **ธีม light ถาวร** (`garmin\.streamlit\config.toml` — แก้ print สีเพี้ยน). **ACWR ใช้ Garmin training_load ถ้านาฬิกาให้ (พี่เก้า = รวม cross-training) ไม่งั้น fallback ระยะวิ่ง (ต้อง/แดน)**. **`LTHR_BY_SLUG` ต้องอัปเดตหลังเทสใหม่** (tong=171, dan=178, **p'kao=184 จาก Garmin LT 16 ก.ค. 69** — เทสแลบ Lactate ไม่มีค่า HR) |
