...
- **ข้อมูลไม่ครบ ≠ ระบบพัง — เช็ค 3 ชั้นก่อนสรุป:** (1) **ฟิลด์ device-dependent** — `training_readiness`/`acute_load`/`endurance_score`/`hill_score`/`respiration` มีเฉพาะพี่เก้า (ต้อง/แดน 0/31 วัน = ปกติ นาฬิการุ่นเก่ากว่า) (2) **ฟิลด์ที่ Garmin คำนวณเฉพาะเงื่อนไข** — `vo2max_trend` ขึ้นเฉพาะวันที่มีวิ่ง outdoor ที่เข้าเกณฑ์ (พี่เก้า 3/31 วันเพราะซ้อม indoor เป็นหลัก ไม่ใช่บั๊ก) และ `lactate_threshold_*` เขียนเฉพาะวันที่นาฬิกาตรวจ LT ใหม่ (พี่เก้า 2/31 = 16 + 31 ก.ค.) (3) **นักกีฬาไม่ได้ใส่นาฬิกา** — ถ้าคอลัมน์ที่หายเป็น**กลุ่มการนอนทั้งชุด**พร้อมกัน (`sleep_*`, `hrv_last_night`, `bb_at_wake`, `bb_during_sleep`, `avg_sleep_respiration`) แต่ค่ากลางวัน (steps/stress/RHR) ยังมา = คืนนั้นไม่ได้ใส่นาฬิกานอน ไม่ใช่ปัญหาการดึง — **ยืนยันได้ด้วยการเทียบกับอีก 2 คนในวันเดียวกัน** ถ้าคนอื่นมีครบ = ไม่ใช่ปัญหาฝั่งระบบ
